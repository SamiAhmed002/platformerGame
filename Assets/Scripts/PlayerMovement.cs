...
public class PlayerMovement : MonoBehaviour
{
    ...
    public float movementVolume = 1f;       // Base volume for movement sounds
    ...
    void Update()
    {
        ...
        if (Input.GetButton("Jump") && canMove && characterController.isGrounded)
        {
            ...
            if (jumpSound != null)
            {
                movementAudioSource.PlayOneShot(jumpSound, movementVolume * SettingsManager.soundVolume);
            }
        }
        else
        {
    ...
    void HandleMovementSound(float curSpeedX, float curSpeedY, bool isRunning)
    {
        ...
        if (isMoving && characterController.isGrounded)
        {
            if (!isPlayingMovementSound)
            {
                ...
                movementAudioSource.volume = movementVolume * SettingsManager.soundVolume;
